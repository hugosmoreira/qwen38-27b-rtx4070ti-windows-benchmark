...
class FormalSchemaTests(unittest.TestCase):
    ...
    def test_quality_schema_is_draft_2020_12_and_covers_tasks(self) -> None:
        root = Path(__file__).resolve().parents[1]
        schema = json.loads(
            (root / "schemas/quality-evaluation-result.schema.json").read_text(encoding="utf-8")
        )
        self.assertEqual(schema["$schema"], "https://json-schema.org/draft/2020-12/schema")
        self.assertFalse(schema["additionalProperties"])
        self.assertIn("tasks", schema["required"])
        self.assertIn("task", schema["$defs"])
        self.assertIn("summary", schema["$defs"])
