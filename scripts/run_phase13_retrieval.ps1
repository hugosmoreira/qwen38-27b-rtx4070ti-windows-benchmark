...
param(
    [Parameter(Mandatory)]
    [ValidateSet(
        'configs/phase13-iq4-xs-retrieval-16k-q8.json',
        'configs/phase13-iq4-xs-retrieval-16k-q4.json',
        'configs/phase13-iq4-xs-retrieval-64k-q4.json'
    )]
    [string]$Config,
...
if ([string]$launch.model_manifest -ne 'environment/phase13-iq4-xs-download-manifest.json' -or
    ...
    throw 'The server launch does not match the frozen Phase 13F retrieval configuration.'
}
...
if ($LASTEXITCODE -ne 0) { throw 'Phase 13F retrieval evaluation did not complete every request.' }
