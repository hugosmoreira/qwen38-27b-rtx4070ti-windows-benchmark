...
$manifestPath = Join-Path $repositoryRoot 'environment\model-download-manifest.json'
...
foreach ($requiredPath in @($PromptFile, $manifestPath, $desktopSecretPath)) {
...
if (Test-Path -LiteralPath $tauriLogPath -PathType Leaf) {
    $latestLaunch = Select-String -LiteralPath $tauriLogPath -Pattern 'Starting llama-server:' | Select-Object -Last 1
...
if (-not $expectedModelPath.Equals($activeModelPath, [System.StringComparison]::OrdinalIgnoreCase)) {
    throw "The active model does not match the pinned Phase 1 model. Expected '$expectedModelPath'; found '$activeModelPath'."
}
...
if ($configurationChecks.Values -contains $false) {
    throw "The active model does not match the Phase 1 smoke configuration: $($configurationChecks | ConvertTo-Json -Compress)"
}

$gitCommit = (& git -c "safe.directory=$($repositoryRoot.Replace('\', '/'))" rev-parse HEAD 2>$null | Select-Object -First 1).Trim()
$startedAt = Get-Date
$runId = 'phase1-smoke-' + $startedAt.ToUniversalTime().ToString('yyyyMMddTHHmmssZ')
...
$record = [ordered]@{
    schema_version = 'phase1-smoke-1.0'
    run_id = $runId
    ...
    runtime_record = 'environment/phase1-unsloth-runtime-2026-08-15.json'
