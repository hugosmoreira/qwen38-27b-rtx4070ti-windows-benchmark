...
param(
    [Parameter(Mandatory)]
    [ValidateSet('configs/phase8-quality-iq2.json', 'configs/phase8-quality-q2.json')]
    [string]$Config,
...
& $virtualPython -m qwen_bench quality-run `
    --repository-root $repositoryRoot `
    --config $Config `
    --server-pid $ServerProcessId
if ($LASTEXITCODE -ne 0) { throw 'Phase 8 quality evaluation did not complete every request.' }
